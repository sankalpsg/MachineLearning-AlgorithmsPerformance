# -*- coding: utf-8 -*-
"""
Created on Tue Oct 24 22:33:08 2017
...
def executeAlgo( datasetDirectory , codebaseDir ):
	...
	Accuracy = cross_val_score(estimator= classifier,X= X1, y = y1, cv=10, scoring= "accuracy")
	Precision = cross_val_score(estimator= classifier,X= X1, y = y1, cv=10, scoring= "precision")
	ROC = cross_val_score(estimator= classifier,X= X1, y = y1, cv=10, scoring= "roc_auc")
	Recall = cross_val_score(estimator= classifier,X= X1, y = y1, cv=10, scoring= "recall")
	F1 = cross_val_score(estimator= classifier,X= X1, y = y1, cv=10, scoring= "f1")
